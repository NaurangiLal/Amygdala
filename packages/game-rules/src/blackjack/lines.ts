...
import type { DogLine } from '../types.ts';

// event -> { state, lines } (PRD §8, DogEvent -> DogState/Lines).
// `satisfies` rather than a type annotation: it checks every entry against
// DogLine while keeping the literal event names, so a typo'd event like
// dog.say('you_bustd') is a compile error instead of a silent no-op.
...
} satisfies Record<string, DogLine>;
...
/** Every event the dog knows how to narrate. */
export type DogEvent = keyof typeof BLACKJACK_LINES | keyof typeof SHELL_LINES;
